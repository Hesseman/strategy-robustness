...
## Timing sensitivity app (port 8503)
...
**http://localhost:8503**); `docker compose up timing` starts only this one. Without compose:
...
| `port is already allocated` | Something else uses 8501 (or 8503 for the timing app). In `docker-compose.yml` change the left-hand port, e.g. `127.0.0.1:8501:8501` to `127.0.0.1:8504:8501`, and open http://localhost:8504. |
...
## Develop
    ...
    .venv/Scripts/python -m streamlit run app/timing_app.py --server.port 8503
