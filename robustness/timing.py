...
CAVEAT = ("This is a picture of fragility, not a test with a verdict. A moved leg fills at the open of the bar "
          ...
          "move the curve's level, not its shape.")
...
class DelayCurve:
    ...
    (max_k + 1) % returns, NaN where the trade is skipped."""
    kind: str
    mode: str
...
def delayed_returns(trades: pd.DataFrame, open_: np.ndarray, k: int, leg: str,
                    mode: str = "fixed_exit") -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Move one leg of every trade k bars later and price the result.
    ...
    moved leg fills at open_[idx + k]; the input is not mutated; raises ValueError on a bad
    k, leg or mode."""
...
def delay_curve(trades: pd.DataFrame, open_: np.ndarray, point_value: float, leg: str,
                mode: str = "fixed_exit", max_k: int = 10) -> DelayCurve:
    """Aggregate delayed_returns over k = 0..max_k.
    ...
    for k in range(max_k + 1):
        pct, pts, alive = delayed_returns(trades, open_, k, leg, mode)
        per_trade[:, k] = pct
...
def run_timing(report: ParsedReport, bars: pd.DataFrame, *, max_k: int = 10, mode: str = "fixed_exit",
    ...
    curves, the share of entries filled at the bar open, and the fixed caveat.
